...
const Loginst = ({setStudent}) => {
  ...
  const formik = useFormik({
    ...
    validate,
    onSubmit: values => {
      axios.post("http://localhost:8000/studentlogin",formik.values)
      .then(res=>{
        
         if(res.data.student)
          {
            // setStudent(res.data.user)
            navigate(`/dashboard/${res.data.student._id}`)
          }
          else{
  ...
  return (<div name="signup" className="h-screen xl:px-80 sm:px-52 px-5 bg-black pt-32">
    <H1>Student Login</H1>
    <form action='/studentlogin' method='POST' onSubmit={formik.handleSubmit} >
